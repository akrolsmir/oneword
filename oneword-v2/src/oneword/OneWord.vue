<template>
      ...
      <div class="is-flex is-flex-wrap-wrap is-align-items-center">
        ...
        >
          ...
          <template v-else>
            ...
            <span class="mx-2"
              >{{
                room.roundsInGame == 'Unlimited' ? '&infin;' : room.roundsInGame
              }}
              rounds</span
            >
          </template>
        ...
        <span class="mx-3" v-else>
          ...
          <template
            v-for="category in CATEGORY_ORDER.filter((c) => room.categories[c])"
          >
            <span
              class="comma"
              :class="{
                'has-text-weight-bold': room.currentRound.category == category,
              }"
            >
              {{ category }}</span
            >
  ...
  <template v-for="(round, i) in room.history.slice().reverse()">
    ...
    <a v-if="!correct(round)" class="is-size-7" @click="toggleHistoryCorrect(i)"
      >(Mark as correct)</a
    >
