...
class Node {
  ...
  append(incommingNode) {
    if (!this.isDroppable(incommingNode)) {
      throw new Error(
        `${this.componentName} is not droppable with the incommingNode - ${incommingNode.componentName}.`
      )
    }
...
Node.unserialize = (editor, nodeData, parent = null) => {
  ...
  node.parent = parent
  node.children = nodeData.children.map((data) =>
    Node.unserialize(editor, data, node)
  )
