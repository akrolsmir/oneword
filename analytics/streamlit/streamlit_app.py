...
class FirestoreHelper:
    def __init__(self):
        self.db = firestore.Client.from_service_account_json("firestore-account-key.json")
...
num_rooms = st.sidebar.number_input("Number of rooms to query", min_value=1, max_value=1000, value=1000)
...
min_update_date = datetime.datetime.fromtimestamp(min_update_time / 1000, tz=pytz.utc).date()
max_update_date = datetime.datetime.fromtimestamp(max_update_time / 1000, tz=pytz.utc).date()

start_date = st.sidebar.date_input("Start date filter", min_value=min_update_date, max_value=max_update_date,
                                   value=min_update_date)
end_date = st.sidebar.date_input("End date filter", min_value=start_date, max_value=max_update_date,
                                 value=max_update_date)

exclude_custom_words = st.sidebar.checkbox("Filter out rounds with custom words", value=True)

st.sidebar.subheader("Word Options")
enable_words = st.sidebar.checkbox("Enable Word Processing")
min_rounds_played = st.sidebar.number_input("Minimum Rounds Played", min_value=1, max_value=100, value=10)
min_success_rate_word = st.sidebar.number_input("Minimum Success Rate (Word)", min_value=0.0, max_value=1.0, step=0.01)
bad_word_success_rate = \
    st.sidebar.number_input("Bad word success rate threshold <", min_value=0.0, max_value=1.0, step=0.01, value=0.2)
bad_word_clue_rate = \
    st.sidebar.number_input("Bad word clue rate threshold >", min_value=0.0, max_value=1.0, step=0.01, value=0.15)
bad_word_guess_rate = \
    st.sidebar.number_input("Bad word guess rate threshold >", min_value=0.0, max_value=1.0, step=0.01, value=0.2)

st.sidebar.subheader("Clue Options")
enable_clues = st.sidebar.checkbox("Enable Clue Processing")
min_times_clued = st.sidebar.number_input("Minimum Times Clued", min_value=1, max_value=100, value=10)
min_success_rate_clue = st.sidebar.number_input("Minimum Success Rate (Clue)", min_value=0.0, max_value=1.0, step=0.01)
...
date_filtered_raw_rooms = \
    [room for room in raw_rooms if start_date <= datetime
        .datetime
        .fromtimestamp(room["lastUpdateTime"] / 1000, tz=pytz.utc).date() <= end_date + datetime.timedelta(days=1)]
...
room_id, num_rounds, num_players_cur, num_players_history, date = \
    "id", "number of rounds", "number of players", "number of players history", "date of game"
df = pd.DataFrame([room_to_df(room) for room in filtered_rooms],
...
st.write("Rooms without rounds: " + str(len(date_filtered_raw_rooms) - len(nonempty_rooms)))
st.write("Empty room percentage: " + str(len(nonempty_rooms) / (len(date_filtered_raw_rooms) * 1.0)))
st.dataframe(df)
...
st.line_chart(df_daily[["average_rounds_per_game", "mean_players_per_game", "mean_players_history_per_game"]])
...
st.line_chart(df_monthly[["average_rounds_per_game", "mean_players_per_game", "mean_players_history_per_game"]])
...
category, word, clues, guess, success, no_clues, no_guess = \
    "category", "word", "clues", "guess", "success", 'no clues', 'no guess'
rounds_df = pd.DataFrame(rounds, columns=[category, word, clues, guess, success, no_clues, no_guess])
...
st.write(
    "Rounds without clues percentage: " + str(round_helper.rounds_without_clues / (round_helper.total_rounds * 1.0)))
st.write(
    "Rounds without guesses percentage: " + str(round_helper.rounds_without_guess / (round_helper.total_rounds * 1.0)))

st.write(rounds_df)

df["score"] = df[num_rounds] * df[num_players_cur]
df_num_rounds = df.set_index(num_rounds).groupby(num_rounds).agg(
    number_of_rounds=(room_id, "count"),
    weighted_score=("score", sum)
)
df_num_players = df.set_index(num_players_cur).groupby(num_players_cur).agg(
    number_of_players=(room_id, "count"),
    weighted_score=("score", sum)
)
...
if enable_words:
    ...
    bad_words_df = bad_words_df[word_df["rounds_without_clues_rate"] > bad_word_clue_rate]
    bad_words_df = bad_words_df[word_df["rounds_without_guess_rate"] > bad_word_guess_rate]
